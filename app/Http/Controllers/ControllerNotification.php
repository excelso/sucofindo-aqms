...
class ControllerNotification extends Controller {
    ...
    protected Notification $notifikasi;
    protected NotificationReaded $notifikasi_read;

    public function __construct() {
        $this->notifikasi = new Notification();
        $this->notifikasi_read = new NotificationReaded();
    }

    public function getDataNotifikasi(): JsonResponse {
        try {
            $dataNotifikasi = Notification::dataNotification(request()->user()->id);
            ...
            foreach ($dataNotif as $item) {
                ...
                $data[] = [
                    'id' => $item->id,
                    'title' => $item->title,
                    'message' => Markdown::parse(preg_replace('/[\r\n]/', '', $item->message))->toHtml(),
                    'link' => $link,
    ...
    public function getCountNotifikasi(): JsonResponse {
        try {
            $dataNotifikasi = Notification::dataCountNotification(request()->user()->id);
            return response()->json([
                'message' => 'Notifikasi Ditemukan',
